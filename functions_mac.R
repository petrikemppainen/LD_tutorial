...
get_LD_sim_data <- function(parameters){
  ...
  system(paste0("./fsc_mac -i ./", parameters$folder, "/", parameters$folder, ".par -n 1"))
  ...
  ## do some filtering in plink_mac already here ##
  
  # filter by maf
  
  if(length(parameters$chrom_sizes>22)){
    system(paste0("./plink_mac --file ", path_name, " --make-bed  --maf ", parameters$maf, " --out ", path_name, " --noweb"), ignore.stdout=FALSE)
    system(paste0("cd ", parameters$folder, "\n", "../plink_mac  --bfile ", parameters$folder, "  --write-snplist --noweb"), ignore.stdout=FALSE)
    if(parameters$subsample){
      ...
      if(length(snplist)>parameters$nLoci){
        ...
        system(paste0("./plink_mac --bfile ", path_name, "    --make-bed --extract ", path_name, ".snplist --out ", path_name, " --noweb"), ignore.stdout=TRUE)
      }
    }
    system(paste0("./plink_mac --bfile ", path_name, "  --recode --tab --out ", path_name, " --noweb"), ignore.stdout=TRUE)  
    
  }else{
    system(paste0("./plink_mac --file ", path_name, " --chr-set 32 --make-bed  --maf ", parameters$maf, " --out ", path_name, " --noweb"), ignore.stdout=TRUE)
    system(paste0("cd ", parameters$folder, "\n", "../plink_mac   --chr-set 32 --bfile ", parameters$folder, "  --write-snplist --noweb"), ignore.stdout=TRUE)
    if(parameters$subsample){
      # read snp names
      snplist <- scan(paste0('./', parameters$folder, "/plink.snplist"), quiet = TRUE)
      loci <- sort(sample(snplist, parameters$nLoci))
      write.table(loci, file=paste0(path_name, ".snplist"), quote = FALSE, col.names = F, row.names = F)
      system(paste0("./plink_mac --bfile ", path_name, "   --chr-set 32 --make-bed --extract ", path_name, ".snplist --out ", path_name, " --noweb"), ignore.stdout=TRUE)
    }
    system(paste0("./plink_mac --bfile ", path_name, " --chr-set 32 --recode --tab --out ", path_name, " --noweb"), ignore.stdout=TRUE)  
